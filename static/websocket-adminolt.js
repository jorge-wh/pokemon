...
let div_jobstatic = '<div class="jobstatic-panel" class="modal" style="display: block; position: fixed; z-index: 30; padding-top: 100px; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgb(0,0,0); background-color: rgba(0,0,0,0.1);">'+
...
let div_loading_gif =   '<div id="content-loading-gif" class="loading-gif-adminolt" style="display: none; position: absolute; width: 100%; height: 100%; z-index: 10;">'+
                            '<img width="100" position: absolute; display: inline-block; left: 50%; transform: translateY(-50%);">'+
                        '</div> ';

let wS4Redis_instances = {}

// $("#content-wrapper .page-header").after(div_jobstatic);
$('body').after(div_jobstatic);
$('body').after(div_loading_gif);
...
function receiveMessage(msg) {
    ...
    if(status === "PROGRESS"){
        ...
        */
        ...
        $.ajax({
                type: "GET",
                url: '/pre-message_async/',
                success: function(respuesta) {

                    //console.log("mensajes leidos correctamente")
